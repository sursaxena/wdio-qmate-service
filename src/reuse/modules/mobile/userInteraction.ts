...
export class UserInteraction {
  ...
  /**
   * @function fillOnDevice
   * @memberOf mobile.userInteraction
   * @description Fills the given value into the passed input.
   * @param {Element | string} element - The element describing the element.
   * @param {String |  Number} value - The value to enter.
   * @example await mobile.userInteraction.fillOnDevice(elem, "Service 01");
   */
  async fillOnDevice(element: Element, value: string | number, timeout: number = parseFloat(process.env.QMATE_CUSTOM_TIMEOUT!) || 30000) {
    const vl = this.vlf.initLog(this.fillOnDevice);

    try {
      vl.log(`Validating the value ${value}`);
      validateValue(value);
      vl.log(`Setting the value of element to ${value}`);
      await element.setValue(value);
      vl.log("Given element data is successfully entered on the mobile UI");
    } catch (error) {
